...
export default function OrderDetailPage() {
  ...
  const [searchParams] = useSearchParams(); // Để đọc query param
  const navigate = useNavigate(); // Để xóa query param

  // --- (MỚI) useEffect ĐỂ HIỂN THỊ THÔNG BÁO ---
  useEffect(() => {
    ...
    if (paymentStatus === 'success') {
      ...
      // Xóa query param khỏi URL, để F5 không bị hiện lại
  ...
  // useEffect cũ để tải chi tiết đơn hàng
  useEffect(() => {
    ...
    fetchOrderDetail();
  }, [orderId]); // Chỉ chạy lại khi ID thay đổi
            ...
            <div className="space-y-3">
               <div className="flex items-center gap-2">
                  <span className={`inline-flex items-center gap-1.5 ${paymentStatus.color} px-3 py-1 rounded-full text-xs font-semibold`}>
                   ...
                   {/* Link "Thanh toán ngay" này bạn có thể tạo 1 trang riêng sau */}
